...
import requests
import pandas as pd
from internal_tools.sqlfunctions import param_dic, connect, execute_many
import warnings
...
allcoinslist_df = pd.DataFrame(columns=['Id', 'Symbol', 'CoinName', 'FullName', 'ImageUrl', 'AssetLaunchDate',
                                        'AssetWebsiteUrl'])

for token_key in list(coins_json['Data'].keys()):

    norm_df = pd.json_normalize(coins_json['Data'][token_key])[['Id', 'Symbol', 'CoinName', 'FullName']]
    ...
    try:
        temp_var = coins_json['Data'][token_key]['AssetLaunchDate'].replace(" ", "").replace(" ", "")
...
allcoinslist_df = allcoinslist_df.rename(columns={'Id': 'cryptocompare_id', 'Symbol': 'cryptocompare_symbol',
                                                  'CoinName': 'cryptocompare_coinname', 'FullName': 'cryptocompare_fullname',
...
allcoinslist_df = allcoinslist_df[['cryptocompare_id', 'cryptocompare_symbol', 'cryptocompare_coinname',
                                   'cryptocompare_fullname', 'coingecko_id', 'coingecko_symbol', 'coingecko_name',
                                   'cryptocompare_imageurl', 'cryptocompare_assetlaunchdate',
                                   'cryptocompare_assetwebsiteurl', 'coin_status']]

# print("--- Transform DataFrame: %s seconds ---" % (time.time() - start_time))

# Inserting all df
conn = connect(param_dic)
execute_many(conn, allcoinslist_df, 'public.tokenslist')
conn.close()
