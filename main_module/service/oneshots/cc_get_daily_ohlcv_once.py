import requests
import pandas as pd
from ..internal_tools.sqlfunctions import (
    param_dic,
    connect,
...
for index, row in shortlist_tokens.iterrows():
    ohlcv_json = requests.get(
        URL_OHLCV_DATA + row["cryptocompare_symbol"] + CC_CURR_LIM + CC_API_KEY
    ).json()
    print(ohlcv_json)
    # Get OHLCV Data
    ohlcv_df = ohlcv_df.append(
        cc_transform_ohlcv_1day(
            ohlcv_json, row["cryptocompare_id"], CC_CURR, limit1=False
        )
    )

print("reached line 36", ohlcv_df.values)
conn = connect(param_dic)
execute_many(None, ohlcv_df, "public.cryptocompare_daily_ohlcv")
conn.close()
